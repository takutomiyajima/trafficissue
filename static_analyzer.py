...
def parse_application(badging: str, apk_path: str) -> Dict[str, object]:
    ...
    return {
        ...
        "package_name": package_keys.get("name", ""),
        "version_name": package_keys.get("versionName", ""),
        "version_code": package_keys.get("versionCode", ""),
        "app_label": parse_badging_keys(app_label_line).get("label", ""),
        "min_sdk": parse_badging_keys(sdk_line).get("sdkVersion", ""),
        "target_sdk": parse_badging_keys(target_line).get("targetSdkVersion", ""),
...
def build_json_report(apk_path: str, badging: str, findings: List[StaticFinding], decoded_strings: List[str]) -> Dict[str, object]:
...
def analyze_static(apk_path: str, output_path: str = "logs/static_analysis.csv", json_output_path: Optional[str] = None) -> List[StaticFinding]:
    apk_path = str(Path(apk_path).resolve())
    ...
    if application["sha256"]:
        add("apk", "sha256", str(application["sha256"]), "APK file hash")
    if package_name:
        add("manifest", "package", package_name, "aapt dump badging")
    for key in ("version_name", "version_code", "min_sdk", "target_sdk", "app_label"):
        if application.get(key):
            add("manifest", key, str(application[key]), "aapt dump badging")
    for permission in sorted(set(extract_permissions(badging))):
        category = categorize_permission(permission)
        add("permission", permission.rsplit(".", 1)[-1], permission, f"AndroidManifest.xml category={category['category']} sensitive={category['sensitive']}")
    for component in parse_components(badging):
        add("component", str(component["type"]), str(component["name"]), f"exported={component['exported']} protected_by_permission={component['protected_by_permission']}")
    ...
    for raw_url in sorted(set(URL_RE.findall(raw_data))):
        add("url", "hardcoded_url", safe_decode(raw_url), "APK string table / byte strings")
    for raw_domain in sorted(set(DOMAIN_RE.findall(raw_data))):
    ...
    for category, hits in detect_hints(decoded_strings, SENSITIVE_API_HINTS).items():
        add("sensitive_api_hint", category, ";".join(hits), "APK strings")
    for category, hits in detect_hints(decoded_strings, NETWORK_API_HINTS).items():
        add("network_api_hint", category, ";".join(hits), "APK strings")
    for sdk_name, hits in detect_hints(decoded_strings, SDK_HINTS).items():
        add("sdk_hint", sdk_name, ";".join(hits), "APK strings")
    ...
    if json_output_path:
        ...
        report = build_json_report(apk_path, badging, findings, decoded_strings)
        json_output.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
        print(f"[Static] Saved JSON report to {json_output}")
